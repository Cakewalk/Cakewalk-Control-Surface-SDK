...
CButtonPropsDlg::CButtonPropsDlg( CTacomaSurface* pSurface, CTacomaSurface::ButtonActionDefinition& bad, CWnd* pParent )// = NULL)
...
{
	m_cKeyList.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key);
	m_cShift.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key );
	m_cControl.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key );
	m_cAlt.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key );

	m_cCmdList.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Command );

	m_cTransportList.EnableWindow( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Transport );
}
...
BEGIN_MESSAGE_MAP(CButtonPropsDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_CTRL, &CButtonPropsDlg::OnBnClickedCtrl)
	ON_BN_CLICKED(IDC_ALT, &CButtonPropsDlg::OnBnClickedAlt)
...
{
	m_bad.eActionType= CTacomaSurface::ButtonActionDefinition::AT_Command;
	enableControls();
}

void CButtonPropsDlg::OnBnClickedRadioKey()
{
	m_bad.eActionType = CTacomaSurface::ButtonActionDefinition::AT_Key;
	enableControls();
}

void CButtonPropsDlg::OnBnClickedRadio3()
{
	m_bad.eActionType = CTacomaSurface::ButtonActionDefinition::AT_Transport;
	enableControls();
}
...
//		TRACE( "Cmd %s = %d\n", sz, dwId );
	...
	// fill key list
	int iCur = -1;
	for ( size_t ixK = 0; ixK < CTacomaSurfacePropPage::sm_vKeys.size(); ixK++ )
	{
		CTacomaSurfacePropPage::NamedKey& nk = CTacomaSurfacePropPage::sm_vKeys[ixK];
	...
	if ( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key && iCur != -1 )
		m_cKeyList.SetCurSel( iCur );

	if ( m_bad.wModKeys & CTacomaSurface::SMK_SHIFT )
		m_cShift.SetCheck( TRUE );
	if ( m_bad.wModKeys & CTacomaSurface::SMK_CTRL )
		m_cControl.SetCheck( TRUE );
	if ( m_bad.wModKeys & CTacomaSurface::SMK_ALT )
		m_cAlt.SetCheck( TRUE );

	int id = IDC_RADIO1;
	if ( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Key )
		id = IDC_RADIO2;
	else if ( m_bad.eActionType == CTacomaSurface::ButtonActionDefinition::AT_Transport )
		id = IDC_RADIO3;
...
void CButtonPropsDlg::OnBnClickedShift()
{
	if ( m_cShift.GetCheck() )
		m_bad.wModKeys |= CTacomaSurface::SMK_SHIFT;
	else
		m_bad.wModKeys &= ~CTacomaSurface::SMK_SHIFT;
}

void CButtonPropsDlg::OnBnClickedCtrl()
{
	if ( m_cControl.GetCheck() )
		m_bad.wModKeys |= CTacomaSurface::SMK_CTRL;
	else
		m_bad.wModKeys &= ~CTacomaSurface::SMK_CTRL;
}

void CButtonPropsDlg::OnBnClickedAlt()
{
	if ( m_cAlt.GetCheck() )
		m_bad.wModKeys |= CTacomaSurface::SMK_ALT;
	else
		m_bad.wModKeys &= ~CTacomaSurface::SMK_ALT;
}
